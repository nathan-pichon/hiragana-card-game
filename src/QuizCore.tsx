...
const getRandomChar = (currentChar: HiraganaChar | null): HiraganaChar => {
  const availableChars = hiraganaData.hiragana.filter(
    (char: HiraganaChar) => !currentChar || char.character !== currentChar.character
  );
  return availableChars[Math.floor(Math.random() * availableChars.length)];
};

const generateOptions = (correctChar: HiraganaChar, mode: QuizCoreProps['mode']): string[] => {
  const correctValue = mode === 'hiragana' ? correctChar.romaji : correctChar.character;
  const otherChars = hiraganaData.hiragana
    .filter((char: HiraganaChar) => char.character !== correctChar.character)
    .map((char: HiraganaChar) => mode === 'hiragana' ? char.romaji : char.character);

  const options = [correctValue];
  while (options.length < 3) {
    const randomValue = otherChars[Math.floor(Math.random() * otherChars.length)];
...
const QuizCore: React.FC<QuizCoreProps> = ({ mode, onAnswer, currentScore }) => {
  const [currentChar, setCurrentChar] = useState<HiraganaChar>(() => getRandomChar(null));
  ...
  useEffect(() => {
    setOptions(generateOptions(currentChar, mode === 'mixed' ? currentMode : mode));
  ...
  const handleAnswer = (answer: string) => {
    ...
    const isCorrect = (mode === 'mixed' ? currentMode : mode) === 'hiragana' 
